...
class BaseSpace(object):
    ...
    def __str__(self):
        ...
        '''
        return '\n'.join('%s: volume=%s,\nmesh=%s' % (tag, vol, mesh) for tag, vol, mesh in
                         zip(self.tags, self.volumes, self.meshes))
...
def KSpace(reciprocals, nk=100, segments=None, end=False):
    ...
    mesh = [np.dot([a + (b - a) * i / (nk - 1 if end else nk) for (a, b), i in zip(segments, pos)], reciprocals) for pos
            in it.product(*([range(nk)] * nvectors))]
...
def KPath(path, nk=100, ends=None, mode='R'):
    ...
    return BaseSpace(('k', np.array(
        [start + (stop - start) / (nk - 1 if end else nk) * i for (start, stop), nk, end in zip(path, nks, ends) for i
         in range(nk)])))
