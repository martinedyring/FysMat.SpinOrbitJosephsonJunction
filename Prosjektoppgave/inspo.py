...
class TBA(Engine):
    ...
    def __init__(self, lattice=None, config=None, terms=None, mask=('nambu',), **karg):
        ...
        self.generator = Generator(bonds=lattice.bonds, config=config, table=config.table(mask=mask), terms=terms,
                                   boundary=self.boundary, half=True)
        self.logging()
    ...
    def matrix(self, k=(), **karg):
        ...
        for opt in self.generator.operators:
            ...
            if len(self.mask) == 0:
                i, j = opt.seqs
                if i < nmatrix // 2 and j < nmatrix // 2: result[
                    j + nmatrix // 2, i + nmatrix // 2] += -opt.value * conjugate(phase)
    ...
    def mu(self, filling, kspace=None):
        ...
        float
            The chemical potential of the system.
        '''
        nelectron, eigvals = int(round(filling * (1 if kspace is None else kspace.rank('k')) * self.nmatrix)), sort(
            self.eigvals(kspace))
    ...
    def gse(self, filling, kspace=None):
        ...
        '''
        return sort(self.eigvals(kspace))[
               0:int(round(filling * (1 if kspace is None else kspace.rank('k')) * self.nmatrix))].sum()
...
def TBAGSE(engine, app):
    ...
    engine.log << engine << '\n'
    engine.log << Sheet.fromordereddict(
        {'Total': gse, 'Site': gse / len(engine.lattice) / (1 if app.kspace is None else app.kspace.rank('k'))}) << '\n'
...
def TBABC(engine, app):
    ...
    if app.savedata or app.plot or app.returndata:
        ...
        name = '%s_%s' % (engine, app.name)
        if app.savedata: savetxt('%s/%s.dat' % (engine.dout, name), result)
        if app.plot: app.figure('P', result.reshape((int(sqrt(result.shape[0])), int(sqrt(result.shape[0])), 3)),
                                '%s/%s' % (engine.dout, name), axis='equal')
